			...
			<div class="row">
				...
				foreach ( $taxonomy as $tax ) :
					...
					<div class="col-md-3 col-sm-3 stm-form-1-selects">
						<div class="stm-label heading-font"><?php echo esc_html( stm_get_name_by_slug( $tax ) ); ?>*
						</div>
						<?php if ( $number_field ) : ?>
			...
			<?php foreach ( $data as $data_key => $data_unit ) : ?>
				...
				if ( ! in_array( $data_unit['slug'], $taxonomy, true ) ) :
					...
					<div class="stm-form-1-quarter">
						<?php if ( ! empty( $data_unit['numeric'] ) ) : ?>
							...
							<input
									...
									placeholder="<?php printf( esc_attr__( 'Enter %s %s', 'motors' ), $data_unit['single_name'], ( ! empty( $data_unit['number_field_affix'] ) ) ? '(' . $data_unit['number_field_affix'] . ')' : '' ); //phpcs:ignore ?>"
									<?php echo esc_attr( $is_required ); ?>
							/>
						<?php else : ?>
							<select name="stm_s_s_<?php echo esc_attr( $data_unit['slug'] ); ?>"
									class="add_a_car-select" <?php echo esc_attr( $is_required ); ?>>
								<?php
								$selected = '';
								if ( ! empty( $id ) ) {
								...
								<option value="" <?php selected( $selected, '' ); ?>>
									<?php
										printf(
											/* translators: %s name option */
											esc_html__( 'Select %s', 'motors' ),
											esc_html( $data_unit['single_name'] )
										)
									?>
								</option>
								<?php
			...
			<div class="stn-add-car-location-wrap">
				...
				<div class="stm-form-1-quarter stm_location stm-location-search-unit">
					<div class="stm-location-input-wrap stm-location">
						...
						<input type="text"
							name="stm_location_text"<?php if ( ! empty( $data_value ) ) { ?> class="stm_has_value" <?php }//phpcs:ignore ?>
