...
if ( $restrictions['posts'] < 1 && apply_filters( 'stm_me_get_nuxy_mod', false, 'dealer_pay_per_listing' ) ) {
	$restricted = true;
}

if ( get_post_meta( $id, 'pay_per_listing', true ) && apply_filters( 'stm_me_get_nuxy_mod', false, 'dealer_pay_per_listing' ) ) {
	$restricted = false;
}
...
<div class="stm-form-checking-user">
	<div class="stm-form-inner">
		...
		<div class="stm-not-<?php echo esc_attr( $disabled ); ?>">
			<?php do_action( 'stm_listings_load_template', 'add_car/registration.php', compact( 'stm_title_user', 'stm_text_user', 'link' ) ); ?>
		...
		<?php if ( ! is_user_logged_in() ) : ?>
			<ul class="add-car-btns-wrap">
				...
				<?php endif; ?>
				<?php if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'dealer_pay_per_listing' ) && empty( $id ) ) : ?>
					<li class="btn-ppl">
