...
if ( have_posts() ) :
	...
	if ( function_exists( 'stm_is_multilisting' ) && stm_is_multilisting() && wp_is_mobile() ) {
		$nuxy_mod_option = apply_filters( 'stm_me_get_nuxy_mod', 'grid', get_query_var( 'post_type' ) . '_view_type_mobile' );
	}
...
<?php else : ?>
	...
	<div class="stm-listings-empty">
		...
		<a href="<?php echo esc_url( strtok( $_SERVER['REQUEST_URI'], '?' ) ); ?>" class="stm-listings-empty__button">
	...
	if ( ! $enable_distance && apply_filters( 'stm_me_get_nuxy_mod', false, 'recommend_items_empty_result' ) ) {
