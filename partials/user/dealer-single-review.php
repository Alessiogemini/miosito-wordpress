...
if ( ! empty( $user_added ) ) :
	...
	/*Get enabled reviews*/
	$rate_1_label = apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Customer Service', 'motors' ), 'dealer_rate_1' );
	$rate_2_label = apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Buying Process', 'motors' ), 'dealer_rate_2' );
	$rate_3_label = apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Overall Experience', 'motors' ), 'dealer_rate_3' );
	...
	<div class="stm-comment-dealer-wrapper animated fadeIn">
		<?php if ( ! empty( $rate1 ) || ! empty( $rate2 ) || ! empty( $rate3 ) ) : ?>
