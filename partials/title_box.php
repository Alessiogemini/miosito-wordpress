...
if ( is_home() ) {
	...
	$stm_title   = apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Inventory', 'motors' ), 'classic_listing_title' );
	$breadcrumbs = 'hide';
} elseif ( is_category() ) {
	$stm_title = single_cat_title( '', false );
} elseif ( is_tax() ) {
...
if ( apply_filters( 'stm_is_listing', false ) ) {
	$assigned_add_car_page = apply_filters( 'stm_me_get_nuxy_mod', 1755, 'user_add_car_page' );
	$prices_page           = apply_filters( 'stm_me_get_nuxy_mod', 1678, 'pricing_link' );
	...
	$pricing_plans_enabled = apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_plans' );
}
