...
if ( ! function_exists( 'stm_listings_multi_type' ) ) {
	...
	function stm_listings_multi_type( $include_default = false ) {
		...
		if ( $include_default ) {
			array_push( $post_types, apply_filters( 'stm_listings_post_type', 'listings' ) );
		}
...
if ( ! function_exists( 'stm_ajax_buy_car_online' ) ) {
	...
	function stm_ajax_buy_car_online() {
		...
		if ( ! empty( $car_id ) && ! empty( $price ) ) {
			if ( class_exists( 'WooCommerce' ) && apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_woo_online' ) ) {
...
function stm_frontend_javascript_variables() {
	...
	if ( apply_filters( 'stm_get_user_role', false ) && apply_filters( 'stm_me_get_nuxy_mod', false, 'allow_dealer_add_new_category' ) ) {
