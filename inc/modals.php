...
function motors_modals() {
	...
	if ( ! apply_filters( 'stm_is_auto_parts', true ) && ! apply_filters( 'stm_is_rental', true ) ) {
		if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'show_trade_in' ) || ( $elementor_exists && $wp_query->get( 'show_trade_in' ) ) ) {
			get_template_part( 'partials/modals/trade-in' );
		}

		if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'show_offer_price' ) || ( $elementor_exists && $wp_query->get( 'show_offer_price' ) ) ) {
			get_template_part( 'partials/modals/trade-offer' );
		}

		if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'show_calculator' ) || ( $elementor_exists && $wp_query->get( 'show_calculator' ) ) ) {
	...
	$show_listing_test_drive = apply_filters( 'stm_me_get_nuxy_mod', false, 'show_listing_test_drive' );
	$show_test_drive         = apply_filters( 'stm_me_get_nuxy_mod', false, 'show_test_drive' ) || ( $elementor_exists && $wp_query->get( 'show_test_drive' ) );
