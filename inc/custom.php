...
if ( ! function_exists( 'stm_body_class' ) ) {
	function stm_body_class( $classes ) {
		...
		if ( apply_filters( 'stm_is_aircrafts', false ) ) {
			...
			if ( ! empty( $wp_query->post ) && intval( apply_filters( 'stm_me_get_nuxy_mod', 0, 'listing_archive' ) ) === $wp_query->post->ID ) {
		...
		$galleries_hoverable = apply_filters( 'stm_me_get_nuxy_mod', false, 'gallery_hover_interaction' );
...
if ( ! function_exists( 'stm_listings_user_defined_filter_page' ) ) {
	function stm_listings_user_defined_filter_page() {
		$listing_archive = apply_filters( 'stm_me_get_nuxy_mod', false, 'listing_archive' );
...
if ( ! function_exists( 'stm_show_filter_by_location' ) ) {
	function stm_show_filter_by_location( $orderby, $query ) {
		$lat_from = 0;
		...
		return apply_filters( 'stm_listings_clauses_filter', $orderby, $query );
	}
}
...
if ( ! function_exists( 'stm_modify_query_location' ) ) {
	function stm_modify_query_location() {
		if ( stm_location_validates() ) {
			...
			add_filter( 'posts_orderby', 'stm_show_filter_by_location', 10, 2 );
		}
	}
}
...
if ( ! function_exists( 'stm_generate_title_from_slugs' ) ) {
	function stm_generate_title_from_slugs( $title, $post_id, $show_labels = false ) {
		...
		$title_from = apply_filters( 'stm_me_get_nuxy_mod', '', 'listing_directory_title_frontend' );
...
if ( ! function_exists( 'stm_edit_delete_user_car' ) ) {
	function stm_edit_delete_user_car() {
		...
		if ( ! $demo ) {
			...
			$featured_payment_enabled = apply_filters( 'stm_me_get_nuxy_mod', false, 'dealer_payments_for_featured_listing' );

			$featured_listing_price = apply_filters( 'stm_me_get_nuxy_mod', 0, 'featured_listing_price' );
...
if ( ! function_exists( 'stm_get_add_page_url' ) ) {
	function stm_get_add_page_url( $edit = '', $post_id = '' ) {
		if ( get_post_type( $post_id ) === apply_filters( 'stm_listings_post_type', 'listings' ) ) {
			$page_id = apply_filters( 'stm_me_get_nuxy_mod', 1755, 'user_add_car_page' );
		} else {
	...
	function stm_get_dealer_marks( $dealer_id = '' ) {
		if ( ! empty( $dealer_id ) ) {
			...
			$ratings = array(
				'average'     => 0,
				'rate1'       => 0,
				'rate1_label' => apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Customer Service', 'motors' ), 'dealer_rate_1' ),
				'rate2'       => 0,
				'rate2_label' => apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Buying Process', 'motors' ), 'dealer_rate_2' ),
				'rate3'       => 0,
				'rate3_label' => apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Overall Experience', 'motors' ), 'dealer_rate_3' ),
				'likes'       => 0,
				'dislikes'    => 0,
				'count'       => 0,
			);
...
if ( ! function_exists( 'stm_data_binding' ) ) {
	function stm_data_binding( $data = array(), $allowAll = false, $is_add_car = false ) {
		...
		foreach ( $attributes as $attr ) {
			...
			foreach ( apply_filters( 'stm_get_category_by_slug_all', array(), $slug, $is_add_car, true ) as $term ) {
...
if ( ! function_exists( 'stm_payment_enabled' ) ) {
	function stm_payment_enabled() {
		...
		$paypal_email    = apply_filters( 'stm_me_get_nuxy_mod', '', 'paypal_email' );
		$paypal_currency = apply_filters( 'stm_me_get_nuxy_mod', 'USD', 'paypal_currency' );
		$paypal_mode     = apply_filters( 'stm_me_get_nuxy_mod', 'sandbox', 'paypal_mode' );
		$membership_cost = apply_filters( 'stm_me_get_nuxy_mod', '', 'membership_cost' );
...
if ( ! function_exists( 'stm_generatePayment' ) ) {
	...
	function stm_generatePayment() {
		...
		if ( ! empty( $user->ID ) ) {
			...
			$url_args = array(
				'cmd'           => '_xclick',
				'business'      => apply_filters( 'stm_me_get_nuxy_mod', '', 'paypal_email' ),
				'item_name'     => $user->data->user_login,
				'item_number'   => $user_id,
				'amount'        => apply_filters( 'stm_me_get_nuxy_mod', '', 'membership_cost' ),
				'no_shipping'   => '1',
				'no_note'       => '1',
				'currency_code' => apply_filters( 'stm_me_get_nuxy_mod', 'USD', 'paypal_currency' ),
				'bn'            => 'PP%2dBuyNowBF',
				'charset'       => 'UTF%2d8',
				'invoice'       => $user_id,
				'return'        => $return_url,
...
if ( ! function_exists( 'stm_get_dealer_list_page' ) ) {
	function stm_get_dealer_list_page() {
		$dealer_list_page = apply_filters( 'stm_me_get_nuxy_mod', 2173, 'dealer_list_page' );
		...
		if ( 'site_style_default' !== $site_color_style ) {
			...
			if ( 'motorcycle' === $layout ) {
				...
				if ( 'boats' !== $layout ) {
					...
					/*Dealler Two*/
					if ( apply_filters( 'stm_is_dealer_two', false ) || boolval(
						apply_filters(
							'is_listing',
							array(
								'listing_two',
...
if ( ! function_exists( 'stm_boats_styles' ) ) {
	function stm_boats_styles() {
		...
		if ( apply_filters( 'stm_is_motorcycle', false ) || apply_filters( 'stm_is_rental', false ) ) {
			...
			$stm_single_car_page = apply_filters( 'stm_me_get_nuxy_img_src', '', 'stm_single_car_page' );
					...
					if ( ! empty( $taxonomy_info['slider_in_tabs'] ) && $taxonomy_info['slider_in_tabs'] ) {
						...
						if ( ! empty( $taxonomy_info['numeric'] ) && $taxonomy_info['numeric'] ) {
							...
							if ( 'location' === $select_string ) {
								...
								if ( $enable_distance ) {
									$radius  = apply_filters( 'stm_me_get_nuxy_mod', 100, 'distance_search' );
...
function stm_pricing_enabled() {
	$enabled = apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_plans' );
...
function stm_pricing_link() {
	$pricing_link = apply_filters( 'stm_me_get_nuxy_mod', '', 'pricing_link' );
...
if ( ! function_exists( 'stm_listings_filter_classified_title' ) ) {
	function stm_listings_filter_classified_title( $params ) {
		$title_default           = apply_filters( 'stm_me_get_nuxy_mod', esc_html__( 'Cars for sale', 'motors' ), 'listing_directory_title_default' );
...
function stm_listing_pre_get_vehicles( $query_vars ) {
	...
	if ( ! is_admin() ) {
		...
		if ( ! empty( $_GET['stm_keywords'] ) ) {
			$query_vars['stm_keywords'] = sanitize_text_field( $_GET['stm_keywords'] );
		}

		if ( ! empty( $_GET['stm_location_address'] ) ) {
			$query_vars['stm_location_address'] = sanitize_text_field( $_GET['stm_location_address'] );
		}
	}
...
if ( ! function_exists( 'stm_get_sidebar_position' ) ) {
	function stm_get_sidebar_position() {
		$listing_filter_position = apply_filters( 'stm_me_get_nuxy_mod', 'left', 'listing_filter_position' );
...
function stm_enable_listing_search_name() {
	return ( apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_search' ) );
}
	...
	function stm_get_filter_badges() {
		...
		foreach ( $attributes as $attribute => $terms ) {
			...
			if ( ! empty( $options['slider'] ) && $options['slider'] ) {
				...
				if ( ! empty( $_GET[ $attribute ] ) || ( class_exists( \MotorsVehiclesListing\FriendlyUrl::class ) && ! empty( \MotorsVehiclesListing\FriendlyUrl::$for_filter[ $attribute ] ) ) ) {

					$selected = ( ! empty( $_GET[ $attribute ] ) ) ? $_GET[ $attribute ] : '';

					if ( class_exists( \MotorsVehiclesListing\FriendlyUrl::class ) && ! empty( \MotorsVehiclesListing\FriendlyUrl::$for_filter[ $attribute ] ) ) {
						$selected = \MotorsVehiclesListing\FriendlyUrl::$for_filter[ $attribute ];
					}
...
function stm_sticky_admin_enqueue_scripts() {
	...
	if ( ! in_array( $screen->base, array( 'post', 'edit' ), true ) || in_array(
		$screen->post_type,
		array(
			'post',
			'page',
...
add_filter( 'stm_is_multiple_plans', 'stm_is_multiple_plans' );
...
if ( ! function_exists( 'stm_get_listing_archive_page_id' ) ) {
	...
	function stm_get_listing_archive_page_id() {
		...
		return apply_filters( 'stm_me_get_nuxy_mod', null, 'listing_archive' );
	}
}
...
if ( ! function_exists( 'stm_get_inventory_page_url' ) ) {
	function stm_get_inventory_page_url( $default = '', $listing_type = 'listings' ) {
		...
		if ( 'listings' === $listing_type ) {
			...
			$options        = get_option( 'wpcfto_motors_' . $current_layout . '_settings', '' );

			if ( $options && isset( $options['listing_archive'] ) ) {
				$page = get_post( $options['listing_archive'] );
				if ( ! is_null( $page ) ) {
...
function stm_listings_sold_custom_column_data( $column, $post_id ) {
	switch ( $column ) {
		case 'mark_as_sold':
			...
			?>
			<input class="car_mark_as_sold" type="checkbox" name="<?php echo esc_attr( $post_id ); ?>" <?php echo ( ! empty( $sold ) ) ? 'checked' : ''; ?>>
			<?php
			break;
	}
}
...
function stm_run_after_theme_plugin_update( $upgrader_object, $options ) {
	if ( ! empty( $options['action'] ) && 'update' === $options['action'] && in_array(
		$options['type'],
		array(
			'theme',
			'plugin',
