...
class MultiplePlan {
	...
	public function buildPlansMeta() {
		$defaultUserQuota    = intval( apply_filters( 'stm_me_get_nuxy_mod', 3, 'user_post_limit' ) );
		$defaultUserImgQuota = intval( apply_filters( 'stm_me_get_nuxy_mod', 5, 'user_post_images_limit' ) );
		$dealer              = apply_filters( 'stm_get_user_role', false, self::$currentUser );

		if ( $dealer ) {
			$defaultUserQuota    = intval( apply_filters( 'stm_me_get_nuxy_mod', 50, 'dealer_post_limit' ) );
			$defaultUserImgQuota = intval( apply_filters( 'stm_me_get_nuxy_mod', 10, 'dealer_post_images_limit' ) );
		}
