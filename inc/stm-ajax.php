...
if ( ! function_exists( 'stm_submit_review' ) ) {
	function stm_submit_review() {
		...
		if ( ! $error ) {
			...
			if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'dealer_review_moderation' ) ) {
				$post_status = 'pending';
			} else {
				$post_status = 'publish';
			}
...
function stm_load_dealers_list() {
	...
	if ( ! empty( $offset ) ) {
		$dealers = STM_Listing\Modules\Users\UserController::get_dealers_data( array(), $offset, $per_page );
...
function stm_add_a_car() {
	...
	if ( apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_plans' ) && apply_filters( 'stm_is_multiple_plans', false ) && 'pay' !== $_POST['btn-type'] ) {
	...
	if ( ! $error ) {
		...
		if ( ! is_wp_error( $post_id ) ) {
			...
			foreach ( $first_step as $tax => $term ) {
				...
				*/
				if ( array_key_exists( 'listing_taxonomy_parent', $tax_info )
					&& ! empty( $tax_info['listing_taxonomy_parent'] )
					&& array_key_exists( $tax_info['listing_taxonomy_parent'], $first_step ) ) {
			...
			$regular_price_label = apply_filters( 'stm_me_get_nuxy_mod', '', 'addl_price_label' );
			if ( ! empty( $regular_price_label ) ) {
				update_post_meta( $post_id, 'regular_price_label', $regular_price_label );
			}

			$special_price_label = apply_filters( 'stm_me_get_nuxy_mod', '', 'addl_sale_price_label' );
...
function stm_ajax_get_cars_for_inventory_map() {
	...
	foreach ( $response->get_posts() as $k => $val ) {
		...
		if ( ! empty( get_post_meta( $val->ID, 'stm_lat_car_admin' ) ) && 'publish' === $val->post_status || 'private' === $val->post_status ) {
			...
			$price = ( isset( $car_meta['price'] ) ) ? apply_filters( 'stm_filter_price_view', '', $car_meta['price'][0] ) : 0 . stm_get_price_currency();
